...
public class NetworkTool
{
    ...
    public static byte[] NetSerialize<T>(T obj, int size = 128) where T : INetworkSerializable, new()
    {
        ...
        try
        {
            FastBufferWriter writer = new FastBufferWriter(size, Allocator.Temp, size);
    ...
    //Serialize an  using Netcode > //Deserialize a [System.Serializable] from bytes
    public static void NetSerialize<T, TS>(BufferSerializer<TS> serializer, ref T obj) where TS : IReaderWriter where T : class
    {
        if (serializer.IsReader)
        {
            int size = 0;
            if (!serializer.GetFastBufferReader().TryBeginRead(sizeof(int)))
            {
                return;
            }
            serializer.SerializeValue(ref size);
#if UNITY_EDITOR
            Debug.Log($"Size {typeof(T)} : {size}");
#endif
            if (size > 0)
            {
        ...
        if (serializer.IsWriter)
        {
            ...
            if (obj == null) return;
            byte[] bytes = NetworkTool.Serialize(obj);
...
{
    public static string ReadValueSafe(this FastBufferReader reader)
    {
        reader.TryBeginRead(reader.Length);
        reader.ReadValue(out string s);
        return s;
    }
}
