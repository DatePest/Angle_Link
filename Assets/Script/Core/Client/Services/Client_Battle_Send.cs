...
namespace Client
{
    public class Client_Battle_Send : IRequestSend_Client
    {
        ...
        {
            SendAction(NetworkMsg_HandlerTag.RequestBattleStart, ApiTool.ToJson(Sdata.data), TargetID, NetworkDelivery.ReliableFragmentedSequenced);
        }
        [EventTag(ClientEventTag.SendSelectToServerRequest)]
        public void SendSelectToServerRequest(NetSendData Sdata)  
        {
            SendAction(NetworkMsg_HandlerTag.RequestBattlePlayerSelect, ApiTool.ToJson(Sdata.data), TargetID, NetworkDelivery.ReliableFragmentedSequenced);
        }
        [EventTag(ClientEventTag.SendBattleEndSettlementRequest)]
        public void SendBattleEndReqDrop(NetSendData Sdata)
        {
            SendAction(NetworkMsg_HandlerTag.RequestBattleEndSettlement, ApiTool.ToJson(Sdata.data), TargetID, NetworkDelivery.ReliableFragmentedSequenced);
        }
        [EventTag(ClientEventTag.SendBattleData)]
        public void SendBattleData(NetSendData Sdata)
        {
            SendAction(NetworkMsg_HandlerTag.RequestReceiveBattleData, ApiTool.ToJson(Sdata.data), TargetID, NetworkDelivery.ReliableFragmentedSequenced);
        }
