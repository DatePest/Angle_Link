...
public class NetworkMessaging 
{
    ...
    public static NetworkMessaging Get() => Network.Get().Messaging;
    public NetworkMessaging(Network network)
    {
    ...
    public void UnListenMsg(string type)
    {
        ...
        if (network.NetManager.CustomMessagingManager != null)
            network.NetManager.CustomMessagingManager.UnregisterNamedMessageHandler(type);
    }
    ...
    private void RegisterNetMsg(string type, System.Action<ulong, FastBufferReader> callback)
    {
        if (IsOnline())
        {
            network.NetManager.CustomMessagingManager.RegisterNamedMessageHandler(type, (ulong client_id, FastBufferReader reader) =>
            {
    ...
    {
        Debug.Log($"ReceiveNetMessage {type}");
    ...
    {
        FastBufferWriter writer = new FastBufferWriter(msg.Length, Allocator.Temp, Network.Msg_size);
    ...
    {
        FastBufferWriter writer = new FastBufferWriter(256, Allocator.Temp, Network.Msg_size);
    ...
    {
        network.NetManager.CustomMessagingManager.SendNamedMessage(type, target, writer, delivery);
    }

    private void SendOnline(string type, IReadOnlyList<ulong> targets, FastBufferWriter writer, NetworkDelivery delivery)
    {
        network.NetManager.CustomMessagingManager.SendNamedMessage(type, targets, writer, delivery);
    }
        ...
        if (IsServer() && IsOnline())
        {
            ...
            foreach (ulong client in ClientList())
            {
                if (client != source_client && client != ClientID())
                    network.NetManager.CustomMessagingManager.SendNamedMessage(type, client, writer, delivery);
            }
            writer.Dispose();
        }
    }
